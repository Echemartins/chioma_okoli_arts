...
export default function Home() {
  ...
  const lightmode = (
    <div className="bg-white flex flex-col items-center md:mb-32 text-gray-800">
        ...
        <nav className="hidden md:flex gap-8 ml-10 items-center">
          {navLinks.map(({ label, href }) => (
            <Link
              key={href}
              href={href}
              className={`text-sm font-medium hover:text-orange-500 transition ${pathname === href ? "text-orange-600 font-semibold" : ""
                }`}
            >
              {label}
            </Link>
      ...
      <div
        className={`md:hidden fixed top-0 left-0 right-0 bottom-0 bg-white bg-opacity-95 text-xl text-gray-800 z-50 transition-transform duration-300 ease-in-out ${isMenuOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        ...
        <nav className="flex flex-col items-center mt-10 space-y-6">
          {navLinks.map(({ label, href }) => (
            <Link
              ...
              href={href}
              onClick={closeMenu}
              className={`text-lg hover:text-orange-500 transition ${pathname === href ? "text-orange-600 font-semibold" : ""
                }`}
            >
              {label}
            </Link>
  ...
  const darkMode = (
    <div className="mt-12 bg-black flex flex-col items-center text-white">
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden fixed top-0 left-0 right-0 bottom-0 bg-black text-2xl text-white bg-opacity-80 z-70 transition-transform duration-300 ease-in-out ${isMenuOpen ? "transform translate-x-0" : "transform translate-x-full"
          }`}
      >
