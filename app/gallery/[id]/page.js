...
export default function ArtworkDetail() {
  ...
  if (!artwork) return <div className="text-center text-white mt-10">Loading...</div>;
  ...
  return (
    ...
    <div className="min-h-screen bg-black text-white p-6">
      <div className=" max-w-4xl mx-auto ">
        <Image src={artwork.imageUrl} width={500} height={400} alt={artwork.title} className="rounded-lg h-[calc(100vh-200px)] w-[100%] " />
        <div className="flex flex-col">
        <h1 className="text-3xl font-bold mt-4">{artwork.title}</h1>
        {/* <p className="text-gray-400 mt-2">{artwork.description}</p> */}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-red bg-opacity-80 flex items-center justify-center z-50">
          <form
            onSubmit={handleSubmit}
            className="bg-purple-200 text-black p-8 rounded-lg w-[80%] max-w-lg"
          >
            <h2 className="text-xl font-semibold mb-4 text-gray-600">Purchase Request</h2>
            <input
              type="email"
              name="email"
